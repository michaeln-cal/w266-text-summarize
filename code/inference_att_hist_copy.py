...
class BeamSearchDecoder(object):
  ...
  def decode(self):
    """Decode examples until data is exhausted (if PARAMS.single_pass) and return, or decode indefinitely, loading latest checkpoint at regular intervals"""
    t0 = time.time()
    counter = 0
    while True:
  ...
  def write_for_rouge(self, reference_sents, decoded_words, ex_index):
    ...
    with open(ref_file, "w") as f:
      for idx,sent in enumerate(reference_sents):
        f.write(str(sent.encode('utf8'))) if idx==len(reference_sents)-1 else f.write(str(sent.encode('utf8')+b"\n"))
    with open(decoded_file, "w") as f:
      for idx,sent in enumerate(decoded_sents):
        f.write(str(sent.encode('utf8'))) if idx==len(decoded_sents)-1 else f.write(str(sent.encode('utf8')+b"\n"))
...
def get_decode_dir_name(ckpt_name):
  ...
  else: raise ValueError("PARAMS.data_path %s should contain one of train, val or test" % (FLAGS.data_path))
