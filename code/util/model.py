...
class SummarizationModel(object):
  ...
  def __init__(self,iterator, hps, vocab):
    ...
    self._enc_lens = iterator.article[1]
    self._enc_padding_mask = iterator.enc_mask
    self._enc_batch_extend_vocab =iterator.article[0]

    self._dec_batch = iterator.dec_input[0]
  ...
  # def _make_feed_dict(self, iterator, just_enc=False):
  ...
  def _add_encoder(self, encoder_inputs, seq_len):
    ...
    with tf.variable_scope('encoder'):
      ...
      print("encoder input shape is", encoder_inputs.get_shape()[0].value)
  ...
  def _add_seq2seq(self):
    ...
    hps = self._hps
    vsize = self._vocab[1] # size of the vocabulary
  ...
  def run_encoder(self, sess):
    ...
    """
    # self._make_feed_dict(iterator, just_enc=True) # feed the batch into the placeholders

    (enc_states, dec_in_state, global_step) = sess.run([self._enc_states, self._dec_in_state, self.global_step]) # run the encoder
