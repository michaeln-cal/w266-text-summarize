...
def read_article_file(filename):
    ...
    while True:
        ...
        article_text.append(example.features.feature['article'].bytes_list.value[
...
def read_abstract_file(filename):
    ...
    while True:
        ...
        abstract_text.append(
            abstract2sents(str(example.features.feature['abstract'].bytes_list.value[0]), SENTENCE_START,
...
def count_records(file_dir, filename):
    # if not tf.gfile.Exists(write_dir): tf.gfile.MakeDirs(write_dir)
    count = 0
...
if __name__ == '__main__':
    ...
    for file in os.listdir(target_dir):
        if file.startswith('train_'):
            i = count_records(target_dir, file)
            count = count + i
            print(file, i)
