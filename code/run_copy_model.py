...
# ==============================================================================
# This is for training full attention history and copy model
...
tf.app.flags.DEFINE_boolean('rouge_eval_only', False, 'If True, use source-copy model. If False, use baseline model.')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
    ...
    with sess_context_manager as sess:
        if PARAMS.debug:  # start the tensorflow debugger
...
def run_eval(model, batcher, vocab):
    ...
    sess = tf.Session(config=utils.get_config())
    eval_dir = os.path.join(PARAMS.log_root, "eval")  # make a subdir of the root dir for eval data
...
def main(unused_argv):
    ...
    hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm',
                   ...
                   'copy_source', 'rouge_eval_only', 'eval_path']
    hps_dict = {}
