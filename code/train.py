...
def run_sample_decode(infer_model, infer_sess, model_dir, hps,
                      summary_writer, src_data, tgt_data):
  """Sample decode a random sentence ."""
  with infer_model.graph.as_default():
    loaded_infer_model, global_step = model_helper.create_or_load_model(
        infer_model.model, model_dir, infer_sess, "infer")

  _sample_decode(loaded_infer_model, global_step, infer_sess, hps,
...
def run_internal_eval(
  ...
  if use_test_set and hps.test_prefix:
      ...
      test_src_files, test_tgt_files = data.get_files(hps.data_dir,hps.test_prefix)
...
def run_external_eval(infer_model, infer_sess, model_dir, hps,
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
  ...
  run_sample_decode(infer_model, infer_sess, model_dir, hps, summary_writer,
                    sample_src_data, sample_tgt_data)
  # dev_ppl, test_ppl= 100, 100
  dev_ppl, test_ppl = run_internal_eval(
      eval_model, eval_sess, model_dir, hps, summary_writer)
  dev_scores, test_scores, global_step = run_external_eval(
      infer_model, infer_sess, model_dir, hps, summary_writer)

  result_summary = _format_results("dev", dev_ppl, dev_scores, hps.metrics)
  if hps.test_prefix:
...
def check_stats(stats, global_step, steps_per_stats, hps, log_f):
...
def _get_best_results(hps):
  """Summary of the current best results."""
  tokens = []
  for metric in hps.metrics:
    tokens.append("%s %.2f" % (metric, getattr(hps, "best_" + metric)))
  return ", ".join(tokens)
...
def _sample_decode(model, global_step, sess, hps, iterator,src_data,tgt_data,iterator_src_placeholder,
  ...
  outputs, attention_summary = model.decode(sess)
...
def _external_eval(model, global_step, sess, hps, iterator,
    ...
    for metric in hps.metrics:
      ...
      if save_on_best and scores[metric] > getattr(hps, "best_" + metric):
        ...
        model.saver.save(
            sess,
            os.path.join(
                getattr(hps, "best_" + metric + "_dir"), "summarized.ckpt"),
            global_step=model.global_step)
    utils.save_hparams(out_dir, hps)
  return scores
