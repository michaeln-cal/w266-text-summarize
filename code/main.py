...
def setup_training(model):
  ...
  try:
    run_training(model, sess_context_manager, sv, summary_writer) # this is an infinite loop until interrupted
...
def run_eval(model, iterator, vocab):
  """Repeatedly runs eval iterations, logging to screen and writing summaries. Saves the model with the best loss seen so far."""
  model.build_graph(iterator) # build the graph
  ...
  while True:
    ...
    t0=time.time()
    results = model.run_eval_step(sess, iterator)
    t1=time.time()
