...
local cmd = onmt.utils.ExtendedCmdLine.new('predict.lua')

local options = {
  {
    '-src', '',
    [[Source sequences to predict.]],
    {
...
onmt.predict.Translator.declareOpts(cmd)
...
onmt.predict.Translator.expandOpts(cmd, "bitext")
...
cmd:option('-time', false, [[Measure average prediction time.]])
...
local function main()
  ...
  local predictor = onmt.predict.Translator.new(opt)

  local srcReader = onmt.utils.FileReader.new(opt.src, opt.idx_files, predictor:srcFeat())
  local srcBatch = {}
  local srcIdBatch = {}
  ...
  while true do
    ...
    if srcSeq then
      ...
      table.insert(srcBatch, predictor:buildInput(srcSeq))
      table.insert(srcIdBatch, srcSeqId)

      if withGoldScore then
        table.insert(goldBatch, predictor:buildInputGold(goldOutputSeq))
      end
    ...
    if srcSeq == nil or #srcBatch == opt.batch_size then
      ...
      local results = predictor:predict(srcBatch, goldBatch)
      ...
      for b = 1, #results do
        if (srcBatch[b].words and #srcBatch[b].words == 0
          ...
          if srcBatch[b].words then
            _G.logger:info('SENT %d: %s', sentId, predictor:buildOutput(srcBatch[b]))
          else
            _G.logger:info('FEATS %d: IDX - %s - SIZE %d', sentId, srcIdBatch[b], srcBatch[b].vectors:size(1))
          end

          if withGoldScore then
            _G.logger:info('GOLD %d: %s', sentId, predictor:buildOutput(goldBatch[b]), results[b].goldScore)
          ...
          if opt.dump_input_encoding then
            ...
            for n = 1, #results[b].preds do
              local sentence = predictor:buildOutput(results[b].preds[n])
              ...
              if withAttention then
                ...
                if predictor:srcFeat() then
                  ...
                  local source = predictor:buildOutput(srcBatch[b])
  ...
  if opt.time then
    ...
    _G.logger:info("Average sentence prediction time (in seconds):\n")
  ...
  if opt.save_beam_to:len() > 0 then
    predictor:saveBeamHistories(opt.save_beam_to)
  end
